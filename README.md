# 프로모션 자동화 — 사용 가이드 (v3.9)

Figma 에서 시안 → 추출 → UTF-8 zip → Way(Jira) 이슈 첨부 + 댓글 등록까지, 사업부 PPT 를 Figma 로 자동 가져오기, **PPT 영역 텍스트 → 팝업 컴포넌트 자동 적용 (다중 종류 지원)** 까지 한 흐름으로 처리.
...
- `img`, `이미지`, `(... 이미지)` 같은 PPT placeholder 자동 제외
