...
**v4.1 신규 기능 — Way assignee 이슈 리스트**
- 상단 Way 이슈 키 input 옆 **`[내 이슈 ▾]`** 버튼 → 본인 assignee 미해결 이슈 최대 30개 (updated 최근순) 표시 → 클릭하면 이슈키 자동 입력. 이슈 번호 외우거나 브라우저로 확인할 필요 없음.
...
- **라이브러리 탭 항목만 사용** — 사이트의 `favorite=true` (라이브러리) 만 후보로. `생성 피드` 는 자동 제외
...
- **흰 배경 자동 투명 처리 (개선)** — 코너 + 변 중앙 8개 seed floodfill (thresh=32) 로 배경 제거 + anti-alias fringe alpha 선형 감쇠(soften=40) 로 halo 없이 부드러운 edge. 오브젝트 안쪽 흰색은 유지
...
2. Plugin 실행 → Way 이슈 키 입력 (예: `KONASALES-72525`)
   - **또는** input 옆의 **`[내 이슈 ▾]`** 버튼 클릭 → 본인 assignee 미해결 이슈 최대 30개 (updated 최근순) 리스트에서 골라서 자동 입력 (helper `/issue/my-assigned` 호출, JQL `assignee = currentUser() AND resolution = Unresolved`)
...
- PLATFORM-8 의 **`라이브러리` 탭 항목만** (사이트에서 `favorite` 표시된 것) — `생성 피드` 항목은 자동 제외
...
**자동 후처리 (AI 생성과 동일 룰)**
- 흰 배경 자동 투명화 — 코너 4개 + 변 중앙 4개 총 8개 seed 에서 floodfill (thresh=32) 로 연결된 near-white 영역 제거 → 오브젝트 안쪽 흰색은 유지
- Anti-alias fringe 제거 — 남은 near-white 픽셀 (`min(R,G,B) > 215`) 의 alpha 를 밝기에 비례해 선형 감쇠 (soften=40) → halo 없이 부드러운 페더 edge
