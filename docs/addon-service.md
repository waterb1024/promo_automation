...
| `sotong-bottom` | 소통참여 하단 (480×348) | `banner_소통참여_{서비스명}_bottom_480x348` (**이미지 생성 없음 · 텍스트만 적용**) |
...
| `support-bottom` | `buildAddonSupportBottom` | `banner_benefits_{svc}_bottom_984x264` | Figma 149:5758 반영 · icon(56×56, **ellipse 원형 pastel 배경 = 고정** + image 40×40) · txt-con 244 (VERTICAL auto-layout gap 2) — sub_text 13/18 Regular #666 (NONE 244×18 단일 라인) + main_row (HORIZONTAL auto-layout gap 8) 안에 TEXT 16/22 Bold #000 (W_AND_H) + **tag chip** (auto-layout, padding 6/2, rounded 12) · **placeholder 텍스트 3개 모두 예시로 채워 스펙 텍스트가 부족해도 템플릿 레이아웃 유지** · apply 시 tag bg = image pastel, tag_text = saturated (ellipse 는 변경 안 함) |
| `sotong-bottom` | `buildAddonSotongBottom` | `banner_소통참여_{svc}_bottom_480x348` | Figma 170:5191 반영 · outer(흰 bg + `#eee` 1px stroke, rounded 12, VERTICAL auto-layout padding 16 gap 8 primary CENTER) · **head** (HORIZONTAL auto-layout gap 4 items-center): `img/20/service` 20×20 pastel `#ffcece` (고정 placeholder) + `svc_name` Pretendard Regular 13/18 #333 · **txt** (VERTICAL auto-layout gap 0 layoutGrow 1 primary MAX=justify-end, counter STRETCH): `sub_text` Regular 16/22 #000 + `TEXT` Bold 16/22 #000 (lineHeight 22 로 자연 간격 확보) · **이미지 슬롯·버튼 없음** |
...
|---|---|---|---|
| `3d` | 홈 상단 · 생활편의 상단 · 생활편의 하단 · 지원금혜택 하단 · 배너·팝업 | `Simple 3D illustration of {subject}, cute and minimal, no text, no letters, smooth matte plastic texture, isolated subject on transparent background, no shadow ground plane, high quality, 3D render` | transparent |
...
**소통참여 하단 (`addon-sotong-bottom`) 은 이미지 생성이 아예 트리거되지 않으므로 이 apply 흐름을 타지 않는다.** 기획 셀 선택 → `addon_from_spec` 만 실행되고 텍스트만 채워지고 종료. `_chainImageGenAfterApply` · `image_generate_prepare` 둘 다 skip. 기존 인스턴스 재선택 케이스는 경고 로그 후 no-op.

`applyMode` 가 `addon-*` 계열 (`addon-home-top`, `addon-home-middle`, `addon-life-top`, `addon-life-bottom`, `addon-support-bottom`) 이면 아래가 모두 적용됨:
...
3. **프레임 배경색**: 외곽 프레임 fill 을 SOLID pastel + `visible: false` 로 설정 (레이어 눈은 꺼짐). 예외들:
   - **addon-home-middle**: 외곽 프레임 대신 내부 `bg` descendant 에 pastel 을 `visible:true` 로 적용 (`bgDescendantAddons` 화이트리스트). 하단 흰색 텍스트 카드가 outer 를 덮기 때문에 별도 파스텔 카드가 필요.
   - **addon-life-top**: 외곽 프레임 자체가 파스텔 카드이므로 `visible:true` 로 유지 (`outerVisibleAddons` 화이트리스트).
   - **addon-support-bottom**: 외곽은 흰색 유지. 아이콘 원형 배경(`ellipse`)은 **고정 pastel** (사용자 지시). `tag` descendant 는 pastel(`background_color`), `tag_text` descendant 는 saturated(`button_color`) 로 채색해 chip 컬러만 이미지에서 도출.
...
### 시나리오 4 · 소통참여 하단 · 텍스트만 적용 (이미지 생성 없음)
1. 기획 파일의 소통참여 하단 스펙 셀 선택 (예: `132:21523` 셀 · 서비스명 + sub 문장 + main 문장)
2. 부가서비스 서브탭에서 `소통참여 하단` 선택 → `기획 셀로 텍스트 적용` (버튼 라벨이 자동으로 바뀜)
3. 우측에 새 `banner_소통참여_{svc}_bottom_480x348` 프레임이 생성되고 텍스트 슬롯(svc_name → sub_text → TEXT)이 top-down 으로 채워짐. 아이콘(`img/20/service` 20×20 pastel `#ffcece`) 은 placeholder 그대로 유지 — 사용자가 수동 교체.
...
## Way 업로드 프레임명 규칙 (2026-07-10)

기존 "시안 추출 → Way 업로드" 는 `MMDD_{banner|popup|landing}_{promotion}_{size}` 만 인식했으나, 부가서비스 프레임도 같은 흐름으로 첨부되도록 파서를 확장.

**인식 규칙** (`figma-plugin/code.js` · `parseAddonName`, `parseHomeTopLegacy`)

| 형식 | 예시 | 비고 |
|---|---|---|
| `{image\|banner}_{홈\|생활편의\|benefits\|소통참여}_{svc}_{top\|middle\|bottom}_{w[xh]}[_#hex]` | `image_홈_카카오T택시_top_1080x528`, `banner_benefits_donate_bottom_984x264` | 표준 |
| `home_img_{svc}[_#hex]` | `home_img_donate_#fde8e9` | 홈 상단 legacy — 사이즈 토큰 없음, output 1080×528 고정 |

**Metadata 합성**: MMDD 프리픽스가 없으므로 `date = 오늘 MMDD`, `promotion = svc명` 으로 합성 → zip 파일명 `{MMDD}_{svc}.zip` (예: `0710_donate.zip`).

**댓글 위치별 breakdown**: `extractSelection` 이 addon 프레임마다 한글 위치 라벨(`ADDON_POSITION_LABELS`) 을 `addon_positions` 배열로 metadata 에 실어보내면 helper `_format_counts` 가 "홈 상단 배너1개, 생활편의 하단 배너2개" 형태로 렌더. 라벨은 첫 등장 순서 유지, 위치별 개수 집계.

| category/position | 라벨 |
|---|---|
| `홈/top` | 홈 상단 |
| `홈/middle` | 홈 중단 |
| `생활편의/top` | 생활편의 상단 |
...
**Helper 변경 (`helper/main.py`)**
- `Counts.addon: int = 0` 필드 추가
- `Metadata.addon_positions: Optional[List[str]]` 필드 추가
- `_format_counts(counts, addon_positions)` — addon > 0 이고 positions 제공 시 위치별 breakdown, 미제공 시 `부가서비스N개` fallback
- `SAFE_FILENAME_RE` 에 `#` 허용 — home-top `_#RRGGBB` 색상 접미사 파일명이 허용되지 않은 형식으로 거부되던 문제 해결
...
**소통참여 하단 (`addon-sotong-bottom`) 재작성 — 이미지 생성 skip · 텍스트만 적용**
- Figma 170:5191 반영. outer 160×116, 흰 bg + `#eee` 1px stroke, cornerRadius 12, VERTICAL auto-layout (padding 16, gap 8, primary CENTER, counter MIN).
- `head` (HORIZONTAL auto-layout, gap 4, items-center) = `img/20/service` 20×20 pastel `#ffcece` (placeholder 유지) + `svc_name` Pretendard Regular 13/18 `#333`.
- `txt` (VERTICAL auto-layout, gap 0, layoutGrow 1, layoutAlign STRETCH, primary MAX = justify-end) = `sub_text` Pretendard Regular 16/22 `#000` + `TEXT` Pretendard Bold 16/22 `#000` (lineHeight 22 로 두 줄 사이 자연 간격 확보).
- 기존 `image` 슬롯 · 이 위치의 이미지 생성 로직 전면 제거.
- 단건 흐름 (`_triggerAddonApplyImage`): sotong-bottom 이면 `addon_from_spec` 만 실행하고 `_chainImageGenAfterApply` 를 걸지 않음 → 새 프레임 생성 + 텍스트 채움 후 종료. 기존 인스턴스 재선택 케이스는 경고 로그 후 no-op.
- 배치 흐름 (`_runNextAddonBatchStep`): item.position === "sotong-bottom" 이면 배치 모드가 "전체" 여도 강제 텍스트만 (`itemTextOnly` 도입).
- UI 라벨: 위치 select 값이 sotong-bottom 이면 메인 버튼이 `기획 셀로 텍스트 적용` 으로 바뀜 (`_updateAddonUiByPosition`). "템플릿만 적용" · "미리보기 skip" 은 숨김.
...
**지원금혜택 하단 (`addon-support-bottom`) 재작성** — Figma 149:5758 반영
- 프레임명 `image_지원금혜택_...` → `banner_benefits_{svc}_bottom_984x264` (사이즈 350×88 → 328×88).
- 구조 재설계:
  - 좌측 `icon` 56×56: **`ellipse` 원형 pastel 배경 (고정, 사용자 지시)** + `image` 40×40 (icon slot).
  - 우측 `txt-con` 244 (VERTICAL auto-layout gap 2):
    - `sub_text` 13/18 Regular #666 (NONE 244×18 단일 라인) — placeholder "앱에서 간편하게 이용하세요"
    - `main_row` (HORIZONTAL auto-layout gap 8, items-center):
      - `TEXT` 16/22 Bold #000 (WIDTH_AND_HEIGHT) — placeholder "메인 타이틀"
      - `tag` chip (HORIZONTAL auto-layout, padding 6/2, cornerRadius 12) — placeholder "태그"
  - Button/small 완전 제거 (이 위치는 버튼 없음).
- Placeholder 3개 모두 예시 텍스트로 미리 채움 → 스펙 텍스트가 부족해도 템플릿 시각적 완성도 유지.
- Apply 예외:
  - 외곽 프레임 fill 은 흰색 유지 (pastel routing 대상 아님).
  - `ellipse` 원형 배경은 **고정 pastel** (`#eaf1fb`) — 이미지 dominant color 로 변경 안 됨 (`bg` 아닌 `ellipse` 로 명명해 라우팅 회피).
  - `tag` descendant → pastel(`background_color`) 로 갱신.
  - `tag_text` descendant → saturated(`button_color`) 로 갱신.
  - Button gradient 로직에서 support-bottom 은 skip (버튼 없음).
- `POSITION_EXISTING_RE.support-bottom` = `/^(image_지원금혜택_|banner_benefits_).+_bottom_/` 로 신·구 네이밍 둘 다 인식.
- `_plibExtractPromotion` 이 `banner_benefits` / `지원금혜택` 위치 토큰 인식하도록 확장.
