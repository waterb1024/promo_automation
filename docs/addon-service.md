...
| `sotong-bottom` | `buildAddonSotongBottom` | `banner_소통참여_{svc}_bottom_480x348` | Figma 170:5191 반영 · outer(흰 bg + `#eee` 1px stroke, rounded 12, VERTICAL auto-layout padding 16 gap 8 primary CENTER) · **head** (HORIZONTAL auto-layout gap 4 items-center): `img/20/service` 20×20 pastel `#ffcece` (고정 placeholder) + `svc_name` Pretendard Regular 13/18 #333 · **txt** (VERTICAL auto-layout gap 2 layoutGrow 1 primary MAX=justify-end, counter STRETCH): `sub_text` Regular 16/22 #000 + `TEXT` Bold 16/22 #000 · **이미지 슬롯·버튼 없음** |
...
- `txt` (VERTICAL auto-layout, gap 2, layoutGrow 1, layoutAlign STRETCH, primary MAX = justify-end) = `sub_text` Pretendard Regular 16/22 `#000` + `TEXT` Pretendard Bold 16/22 `#000`.
